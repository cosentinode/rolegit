## Issue

Fixes #

## Summary

<!-- Describe the focused change and why it is needed. -->

## Validation

- [ ] `npm ci`
- [ ] `npm run typecheck`
- [ ] `npm run build`
- [ ] `npm test`
- [ ] `npm run package:check`
- [ ] `npm run smoke`
- [ ] Relevant valid and invalid commitlint behavior was checked
- [ ] Documentation and tests were updated, or are not applicable

## Risks

<!-- Note compatibility, migration, release, or operational risks. Write "None" if none. -->

## Security

<!-- Note effects on key custody, authorization, trust boundaries, sensitive data, or supply chain. Write "None" if none. -->

- [ ] No secrets, plaintext protected files, credentials, or private keys are included
- [ ] Security-sensitive changes were checked against [`docs/threat-model.md`](../docs/threat-model.md)
- [ ] Security and trust-boundary changes are reflected in the relevant documentation and tests, or are not applicable
