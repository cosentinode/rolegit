...
An expiry watcher removes materialized files after the current session expires. `rolegit lock`
removes them immediately, invalidates the server session, and clears the local session token.
